class Array:
    def __init__(self, arr=None, capacity=10) -> None:
    ...
    def _reseize(self, new_capacity):
    ...
    def add(self, index, e):
        ...
        if not 0 <= index <= self._size:
            raise ValueError(
                'add failed.Require index >0 and index <= array size.')
        if self._size == len(self._data):
            if self._size == 0:
                self._reseize(1)
            else:
                self._reseize(2 * len(self._data))
